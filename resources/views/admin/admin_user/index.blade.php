    ...
    <div class="layui-card-body ">
        <form class="layui-form layui-col-space5" id="search-form">
            <div class="layui-inline layui-show-xs-block">
                <input type="text" name="username"  placeholder="请输用户名" value="{{ request("username") }}" autocomplete="off" class="layui-input">
            </div>
            <div class="layui-inline layui-show-xs-block">
        ...
        <script type="text/html" id="toolbar">
            <div class="layui-btn-container">
                @if(admin_user_can("admin-user.create"))
                    <a class="layui-btn layui-btn-sm" onclick="admin.openLayerForm('{{ route("admin-user.create") }}', '添加', 'POST',  '600px', '550px')"><i class="layui-icon"></i>添加</a>
                @endif
            </div>
        </script>
...
{{--        </script>--}}
        <table  lay-filter="table-hide" style="display: none" lay-data="{height:'full-310', cellMinWidth: 80,toolbar: '#toolbar', limit: {{ $adminUsers->perPage() }} }">
            ...
            <tr>
                ...
                <th lay-data="{field:'admin_name'}">操作用户</th>
            ...
            @foreach ($adminUsers as $adminUser)
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td>{{ $adminUser->status == 1 ? '启用' : '禁用' }}</td>
                    <td>{{ $adminUser->created_at }}</td>
                    <td>{{ $adminUser->updated_at }}</td>
                    <td>
                        @if(admin_user_can("admin-user.edit"))
                            <a class="layui-btn layui-btn-xs"
                                onclick="admin.openLayerForm('{{ route("admin-user.edit", ['admin_user' => $adminUser->id]) }}', '编辑', 'PATCH', '600px', '500px')">编辑</a>
                        @endif
